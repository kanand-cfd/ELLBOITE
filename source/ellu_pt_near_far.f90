...
real(kind=8), intent(in)  :: c(n), g(n,n), p(n)
real(kind=8), intent(out) :: xnf(n)

integer    :: itmax = 100  !  max. iterations in dgqt
real(kind=8) :: atol = 1.d-12, rtol = 1.d-4  !  tolerances for dgqt

integer    :: info, i, j
real(kind=8) :: gi(n,n), delta, par, f, sig_min, sig_max, r, dist
real(kind=8) :: a(n,n), b(n), ynf(n), z(n), wa1(n), wa2(n), sign
