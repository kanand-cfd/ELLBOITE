...
real(kind=8), intent(in)  :: c1(n), gg1((n*(n+1))/2), c2(n), gg2((n*(n+1))/2), qual
real(kind=8), intent(out) :: xh(n), v(n)
...
real(kind=8) :: atol = 1.d-6, rtol = 1.d-6  !  tolerances for dgqt

integer    :: info, i, j, k, iter
real(kind=8) :: g1(n,n), g2(n,n), c2y(n), y2(n), y2norm, x1(n), x2(n), g2y(n,n), &
